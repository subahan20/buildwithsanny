...
const DailyCycle = () => {
  ...
  return (
    <section id="daily-cycle" className="py-2 md:py-5 bg-gradient-to-b from-[#020617] via-[#030a1f] to-[#020617] text-white relative overflow-hidden">
      ...
      <div className="max-w-[1440px] mx-auto px-8 md:px-12 lg:px-20 relative z-10">
        <div className="mb-5">
          {/* Removed decorative icon per request */}

          <h2 className="text-4xl sm:text-6xl lg:text-7xl font-black tracking-tighter mb-8 text-white">
        ...
        <div className="max-w-6xl space-y-10">
          {steps.map((step, idx) => (
            <div 
              key={idx} 
              className={`flex flex-col md:flex-row items-center gap-zero group transition-all duration-700 ${step.margin}`}
            >
              {/* Giant Index Number */}
              <span className="text-9xl md:text-[12rem] font-medium leading-none text-transparent bg-clip-text bg-gradient-to-b from-yellow-400/60 to-orange-400/20 tracking-tighter opacity-70 group-hover:opacity-100 transition-opacity duration-700 select-none">
                {step.id}
              </span>
              
              <div className="flex-1 bg-slate-900/40 backdrop-blur-xl border border-white/5 rounded-[2.5rem] p-8 md:p-10 shadow-2xl shadow-yellow-950/20 max-w-2xl relative overflow-hidden transition-transform hover:-translate-y-2 group-hover:border-yellow-500/30">
                <div className="absolute top-10 right-10">
                  {step.icon}
                </div>
                
                <div className="relative z-10">
                  <h3 className="text-3xl font-black text-white mb-6 tracking-tight group-hover:text-yellow-400 transition-colors">
                    {step.title}
                  </h3>
                  <p className="text-slate-200 text-lg font-bold leading-relaxed max-w-md">
                    {step.description}
                  </p>
                </div>
              </div>
        ...
        <div className="mt-5 flex flex-col items-center">
          <Button 
            variant="yellow" 
            className="px-16 py-6 shadow-xl shadow-yellow-500/20"
